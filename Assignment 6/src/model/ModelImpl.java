...
public class ModelImpl implements Model {
  ...
  private List<Strategy> mStrategy;
  ...
  public boolean finalisePortfolio(String name) {
    Portfolio temp;
    if (this.type) {
      temp = new PortfolioFlexImpl(name, this.mStocks, this.transactionHistory, this.mStrategy);
    } else {
  ...
  public void retrievePortfolio(String name) throws XPathExpressionException {
    ...
    Portfolio portfolio = fp.retrieveFile(name);
    this.mPortfolios.add(portfolio);
    List<Strategy> strategy = portfolio.getStrategies();
    for (Strategy temp : strategy) {
      if (temp.getLastDate().plusDays(temp.getInterval()).isBefore(temp.getEndDate()) ||
              temp.getLastDate().plusDays(temp.getInterval()).isEqual(temp.getEndDate())) {
        this.dollarCostAveraging(temp.getPlanName(), temp.getValues(),
                temp.getStartDate(), temp.getEndDate(), temp.getLastDate(),
                temp.getInterval(), temp.getInvestmentPrice());
      }
    }
  }
  ...
  public Map<Object, Double> visualize(Portfolio portfolio,
    ...
    for (LocalDate i = startDate; i.isBefore(endDate); i = i.plusDays(1)) {
      double value = 0.0;
      for (Stock s : portfolio.getTransactionHistory()) {
  ...
  @Override
  public void dollarCostAveraging(String planName, HashMap<String, Double[]> values,
                                  LocalDate startDate, LocalDate endDate, LocalDate lastDate,
                                  int interval, double investmentPrice)
          throws IllegalArgumentException {
    LocalDate date = lastDate;
    while ((endDate == null || date.isBefore(endDate)) && date.isBefore(LocalDate.now())) {
      for (String name : values.keySet()) {
        double priceAllottedForStock = (values.get(name)[0] / 100) * investmentPrice;
        this.createStockFlexForDCA(name, priceAllottedForStock, date, values.get(name)[1]);
      }
      date = date.plusDays(interval);
    }
    Strategy strategy = new StrategyImpl(planName, values, investmentPrice,
            interval, startDate, endDate, date.minusDays(interval));
    for (Strategy temp : this.mStrategy) {
      if (temp.getPlanName().equals(strategy.getPlanName())) {
        this.mStrategy.remove(temp);
      }
    }
    this.mStrategy.add(strategy);
  }

  private void createStockFlexForDCA(String name, double priceAllotted,
                                     LocalDate date, double com)
          throws IllegalArgumentException {
    Stock st;
    try {
      st = new StockFlexImpl(name, priceAllotted, date, com);
    } catch (Exception e) {
      throw new IllegalArgumentException();
    }
    for (Stock stock : this.mStocks) {

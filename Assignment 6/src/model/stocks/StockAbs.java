...
public abstract class StockAbs implements Stock {
  ...
  private HashMap<LocalDate, Double[]> history;
  ...
  public StockAbs(String name, double shares, double price) {
    ...
    try {
      this.history.get(LocalDate.now());
    } catch (Exception e) {
   ...
   *
   * @param name          name of the stock.
   * @param priceAllotted price alloted for the stock.
   * @param date          date of purchase.
   */
  public StockAbs(String name, double priceAllotted, LocalDate date) {
    if (name == null || name.length() == 0 || priceAllotted <= 0.0) {
      throw new IllegalArgumentException("Invalid Stock name!!");
    }
    this.stockName = name;
    try {
    ...
    this.noOfShares = priceAllotted / this.getPriceAtDate(date);
    this.price = this.getCostAtDate(date);
  }
  ...
  public double getPriceAtDate(LocalDate date) throws IllegalArgumentException {
    ...
    try {
      result = this.history.get(date)[3];
   ...
   */
  @Override
  public double getCostAtDate(LocalDate date) throws IllegalArgumentException {
    if (date == null) {
      throw new IllegalArgumentException("Date cannot be null.");
    }
    double result = 0;
    ...
    try {
      ...
      return this.getCostAtDate(date.plusDays(1));
    }
    return result;
  }

...
public class ControlImpl implements Control {
  ...
  private void dollarCostAveraging() {
    ...
    while (true) {
      this.commandLine.startStrategy();
      if (readNext().equalsIgnoreCase("1")) {
        String name = appendStockSymbol();
        Double[] values = {getWeightage(), getCommission()};
        list.put(name, values);
        break;
      } else if (readNext().equalsIgnoreCase("f")) {
        if (!this.checkDCA(list)) {
          this.dollarCostAveraging();
        }
        this.commandLine.appendStrategyName();
        String planName = getInput();
        this.commandLine.appendMessage("Enter start date as:");
        startDate = appendDate();
        this.commandLine.appendMessage("Do you have an end date " +
                "for the strategy? Yes/No");
        if (readNext().equalsIgnoreCase("yes")) {
          this.commandLine.appendMessage("Enter end date as:");
          endDate = appendDate();
        }
        interval = this.getInterval();
        investmentAmount = this.getSharePrice();
        this.model.dollarCostAveraging(planName, list, startDate, endDate,
                startDate, interval, investmentAmount);
        return;
      } else {
        this.commandLine.invalidCommand();
      }
  ...
  private boolean checkDCA(HashMap<String, Double[]> strategy) {
    double sum = 0;
    for (String p : strategy.keySet()) {
      sum += strategy.get(p)[0];
    }
    return sum > 99.99;
  }

  private int getInterval() {
    this.commandLine.appendMessage("Enter the interval after " +
            "which amount is to be invested(in terms of number of days): ");
    try {
  ...
  private int shareCheck() {
    ...
    try {
      int temp = Integer.parseInt(readNext());
      if (temp < 0) {
  ...
  private double getWeightage() {
    this.commandLine.appendWeight();
    try {
      double temp = Double.parseDouble(readNext());
      if (temp < 0) {
        throw new NumberFormatException();
      }
      return temp;
